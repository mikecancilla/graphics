...
ShapeData ShapeGenerator::makePlaneVerts(GLuint dimensions)
{
    ...
    for(GLuint i = 0; i < dimensions; i++)
    {
        for(GLuint j = 0; j < dimensions; j++)
        {
            ...
            thisVert.position.x = (float)(j - half);
            thisVert.position.z = (float)(i - half);
...
ShapeData ShapeGenerator::makeSphere(GLuint tesselation)
{
	...
	for (size_t col = 0; col < dimensions; col++)
	{
		...
		for (size_t row = 0; row < dimensions; row++)
		{
			...
			v.position.x = (float)(RADIUS * cos(phi) * sin(theta));
			v.position.y = (float)(RADIUS * sin(phi) * sin(theta));
			v.position.z = (float)(RADIUS * cos(theta));
...
ShapeData ShapeGenerator::makeTorus(GLuint tesselation)
{
	...
	ret.vertices = new Vertex[ret.numVertices];
	float sliceAngle = 360.f / tesselation;
	...
	for (GLuint round1 = 0; round1 < tesselation; round1++)
	{
		...
		glm::mat4 transform =
			glm::rotate(glm::mat4(1.f), (float) RAD(round1 * sliceAngle), glm::vec3(0.0f, 1.0f, 0.0f)) *
...
ShapeData ShapeGenerator::generateNormals(const ShapeData & data)
{
	...
	for (GLuint i = 0; i < data.numVertices; i++)
	{
